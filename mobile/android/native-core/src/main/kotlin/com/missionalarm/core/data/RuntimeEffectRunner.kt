...
fun interface AlarmRuntimeStarter {
  @Throws(PermanentRuntimeStartException::class)
  fun start(instanceId: String)
}

class PermanentRuntimeStartException : IllegalStateException("alarm runtime request is invalid")

/** Executes only START_ALARM_RUNTIME; presentation remains pending for the Alarm Host increment. */
class RuntimeEffectRunner(
  ...
  private val runtimeStarter: AlarmRuntimeStarter,
) {
  fun drain(maxEffects: Int = DEFAULT_MAX_DRAIN): Int {
    ...
    while (processed < maxEffects) {
      ...
      try {
        runtimeStarter.start(instanceId)
        acknowledge(effect, owner)
      } catch (_: PermanentRuntimeStartException) {
  ...
  private companion object {
    const val EFFECT_TYPE = "START_ALARM_RUNTIME"
    ...
    const val PERMANENT_FAILURE = "ALARM_RUNTIME_PERMANENT_FAILURE"
    const val TRANSIENT_FAILURE = "ALARM_RUNTIME_TRANSIENT_FAILURE"
    const val RETRY_EXHAUSTED = "ALARM_RUNTIME_RETRY_EXHAUSTED"
