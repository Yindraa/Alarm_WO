...
fun interface AlarmRuntimeStopper {
  @Throws(PermanentRuntimeStopException::class)
  fun stop(instanceId: String)
}

class PermanentRuntimeStopException : IllegalStateException("alarm runtime stop request is invalid")

/** Executes only instance-scoped STOP_ALARM_RUNTIME effects. */
class RuntimeStopEffectRunner(
  private val database: MissionAlarmDatabase,
  private val wallClock: WallClock,
  private val leaseOwnerGenerator: LeaseOwnerGenerator,
  private val runtimeStopper: AlarmRuntimeStopper,
) {
  fun drain(maxEffects: Int = DEFAULT_MAX_DRAIN): Int {
    ...
    while (processed < maxEffects) {
      ...
      try {
        runtimeStopper.stop(instanceId)
        acknowledge(effect, owner)
      } catch (_: PermanentRuntimeStopException) {
  ...
  private fun acknowledge(effect: RuntimeEffectEntity, owner: String) {
    check(database.reliabilityDao().acknowledge(effect.id, owner, nowMs()) == 1) {
      "runtime stop effect lease was lost"
    }
  }
  ...
  private companion object {
    const val EFFECT_TYPE = "STOP_ALARM_RUNTIME"
    ...
    const val PERMANENT_FAILURE = "ALARM_RUNTIME_STOP_PERMANENT_FAILURE"
    const val TRANSIENT_FAILURE = "ALARM_RUNTIME_STOP_TRANSIENT_FAILURE"
    const val RETRY_EXHAUSTED = "ALARM_RUNTIME_STOP_RETRY_EXHAUSTED"
